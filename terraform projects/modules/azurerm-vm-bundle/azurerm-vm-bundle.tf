...
locals {
  ...
  nsg_objects = local.nsg_objects_pre > 0 ? { for a in [for b, c in range(local.nsg_objects_pre) : {
    ...
    security_rules = can(length(var.nsg_objects[b].no_rules)) ? null : { for d in [for e, f in range(local.nsg_objects_rules_pre) : { //
  ...
  script_path = (regexall("^(.*\\/)?([^\\/]+)\\.ps1$",var.script_name))[0]

  script_commands = length(local.vm_os_names) > 0 ? flatten([for a, b in range(length(local.vm_os_names)) : [
    length([for c in local.merge_objects : c if c.allow_null_version != null && c.os_name == local.vm_os_names[a]]) > 0 ? "${var.script_name} -Location ${var.location} -OS ${local.vm_os_names[a]} -OutputFileName ${local.script_path[0]}/${local.vm_os_names[a]}-skus.json -AllowNoVersions" : "${var.script_name} -Location ${var.location} -OS ${local.vm_os_names[a]} -OutputFileName ${local.script_path[0]}/${local.vm_os_names[a]}-skus.json"
  ]]) : null
...
data "local_file" "vmskus_objects" {
  ...
  filename = "${local.script_path[0]}/${local.vm_os_names[count.index]}-skus.json"
