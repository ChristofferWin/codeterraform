...
locals {
  ...
  subnet_objects = {for each in [for x, y in range(local.subnet_creation_count) : {
    ...
    service_endpoints = x == 0 && (!var.create_bastion || var.bastion_object == null) || x == 1 && (var.var.create_bastion || var.bastion_object != null) && can(length(flatten((local.merge_objects.*.boot_diagnostics).*.network_rules))) ? ["Microsoft.KeyVault", "Microsoft.Storage"] :  x == 0 && (!var.create_bastion || var.bastion_object == null) || x == 1 && (var.var.create_bastion || var.bastion_object != null) ? ["Microsoft.KeyVault"] : null
  }] : each.name => each}
  ...
  subnet_resource_id  = length(azurerm_subnet.subnet_object) > 1 || length(azurerm_subnet.subnet_object) == 1 && var.subnet_resource_id == null || var.subnet_bastion_resource_id == null ? flatten(values(azurerm_subnet.subnet_object).*.id) : local.subnet_data_helper
