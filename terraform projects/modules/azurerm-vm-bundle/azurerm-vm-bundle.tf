...
locals {
  ...
  nic_objects = { for each in [for x, y in local.merge_objects : {
    ...
    subnet_id                     = local.subnet_return_object != null ? [for each in local.subnet_resource_id : each if !contains(split("/", each), "AzureBastionSubnet")][0] : [for each in local.subnet_return_object : each.id if each.name != "AzureBastionSubnet"][0]
