...
locals {
  ...
  fw_rule_objects_pre = !can(local.tp_object.hub_object.network.firewall.no_rules) ? {} : local.tp_object.hub_object.network.firewall.no_rules == null ? {for each in [for a, b in range(2) : { #Must be by itself so that the rule ONLY relies on the GW finishing deploying and not the FW
      ...
      action = "Allow"
      source_addresses = a == 0 ? flatten([for c, d in local.vnet_objects_pre : d.address_spaces if d.name != [for e, f in local.vnet_objects_pre : f.name if e == local.rg_count -1][0]]) : [for c, d in values(local.subnet_objects) : d.address_prefix if length(regexall("mgmt|management", lower(d.name))) > 0][0]
