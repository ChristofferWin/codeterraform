...
locals {
  ...
  vnet_objects_pre = [for a, b in range(local.rg_count) : {
    name = replace(a == local.rg_count -1 && local.tp_object.hub_object.network == null ? local.vnet_base_name : a == local.rg_count -1 && local.tp_object.hub_object.network.vnet_name != null ? local.tp_object.hub_object.network.vnet_name : a == local.rg_count -1 && local.tp_object.hub_object.network.vnet_name == null ? local.vnet_base_name : a != local.rg_count - 1 && local.tp_object.spoke_objects[a].network == null ? replace(local.vnet_base_name, "hub", "spoke${a + 1}") : a != local.rg_count - 1 && local.tp_object.spoke_objects[a].network.vnet_name != null ? local.tp_object.spoke_objects[a].network.vnet_name : replace(local.vnet_base_name, "hub", "spoke${a + 1}"), "(-{2}|^-|-$", "")
  ...
  fw_log_object = !can(local.tp_object.hub_object.network.firewall.no_logs) ? {} : local.tp_object.hub_object.network.firewall.no_logs == null ?  {for each in [for c, d in range(1) : {
    name = replace(local.tp_object.hub_object.network.firewall.log_name != null ? local.tp_object.hub_object.network.firewall.log_name : replace(local.gateway_base_name, "gw", "log-fw"), "/(--)|(^-)|(-$)/", "")
...
resource "azurerm_subnet_route_table_association" "link_route_table_to_subnet_object" {
  ...
  subnet_id = [for a, b in local.subnet_return_helper_objects : b.id if length(regexall(b.name, each.value.name)) > 0][0]
}
