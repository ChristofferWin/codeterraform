...
locals {
  ...
  pip_objects_pre = [for a, b in range(local.pip_count) : {
      name = a == 1 && !can(local.tp_object.hub_object.network.vpn.pip_name) ? replace(local.gateway_base_name, "gw", "gw-pip") : a == 0 && !can(local.tp_object.hub_object.network.vpn.pip_name) ? replace(local.gateway_base_name, "gw", "gw-pip") : a == 1 && local.tp_object.hub_object.network.vpn.pip_name != null ? local.tp_object.hub_object.network.vpn.pip_name : a == 0 && local.tp_object.hub_object.network.vpn.pip_name != null ? local.tp_object.hub_object.network.vpn.pip_name : !can(local.tp_object.hub_object.network.firewall.pip_name) ? replace(local.gateway_base_name, "gw", "fw-pip") : local.tp_object.hub_object.network.firewall.pip_name != null ? local.tp_object.hub_object.network.firewall.pip_name : replace(local.gateway_base_name, "gw", "fw-pip") #The pip name is NOT correct as of now - I just created a setup with ONLY GW but the PIP name is fucking FW instead of GW
