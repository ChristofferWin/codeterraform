...
locals {
  ...
  vnet_cidr_notation = can(local.tp_object.hub_object.address_spaces[0]) ? "/${split("/", local.tp_object.hub_object.address_spaces[0])[1]}" : "/22"
  ...
  vnet_objects_pre = [for a, b in range(local.rg_count) : {
    ...
    address_spaces = a == local.rg_count -1 && !can(local.tp_object.hub_object.network.address_spaces[0]) ? local.vnet_cidr_block : a == local.rg_count -1 && local.tp_object.hub_object.network.address_spaces != null ? local.tp_object.hub_object.network.address_spaces : a == local.rg_count -1 ? [cidrsubnet(local.vnet_cidr_block[0], 32 - tonumber(replace(local.vnet_cidr_notation, "/", "")), 0)] : a != local.rg_count -1 && !can(local.tp_object.spoke_objects[a].network.address_spaces[0]) ? [cidrsubnet(local.vnet_cidr_block[0], 32 - tonumber(replace(local.vnet_cidr_notation, "/", "")), a + 1)] : a == local.rg_count -1 ? null : local.tp_object.spoke_objects[a].network.address_spaces != null ? local.tp_object.spoke_objects[a].network.address_spaces : [cidrsubnet(local.vnet_cidr_block[0], 32 - tonumber(replace(local.vnet_cidr_notation, "/", "")), a + 1)]
    solution_name = a == local.rg_count -1 ? null : can(local.tp_object.spoke_objects[a].solution_name) ? local.tp_object.spoke_objects[a].solution_name : null
